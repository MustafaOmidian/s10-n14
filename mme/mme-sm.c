...
// i think it needs repair in here
        case OGS_GTP1_FORWARD_RELOCATION_RESPONSE_TYPE:

            if (!gtp_message.h.teid_presence) ogs_error("No TEID");

            handle_forward_relocation_response(

                xact, mme_ue, &gtp_message.forward_relocation_response);

            break;


        case OGS_GTP1_FORWARD_RELOCATION_COMPLETE_NOTIFICATION_TYPE:

            if (!gtp_message.h.teid_presence) ogs_error("No TEID");

            handle_forward_relocation_complete_notification(

                xact, mme_ue, &gtp_message.forward_relocation_complete_notification);
         ...
         case OGS_GTP1_FORWARD_RELOCATION_COMPLETE_ACKNOWLEDGE_TYPE:
            ...
            handle_forward_relocation_complete_acknowledge(

                xact, mme_ue, &gtp_message.forward_relocation_complete_acknowledge);
