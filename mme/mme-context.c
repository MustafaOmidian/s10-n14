...
void remove_ue_from_serving_mme(mme_ue_t *mme_ue) {

    ogs_assert(mme_ue);


    // Perform any necessary cleanup of UE-associated resources here

    // This could involve sending a delete session request to the SGW,

    // releasing any bearer resources, and so on.


    // If the UE has an associated eNB context, release it

    enb_ue_t *enb_ue = enb_ue_cycle(mme_ue->enb_ue);

    if (enb_ue) {

        // Send a UE context release command to the eNB
